...
const receivedAtLabel = computed(() => feedback.createdAt ? `${t('staff.activities.feedbacks.cards.receivedAt')} ${formatLastModified(feedback.createdAt)}` : '')
const updatedAtLabel = computed(() => feedback.updatedAt ? `${t('staff.activities.feedbacks.cards.lastSaved')} ${formatTranslatedDateTime(feedback.updatedAt)}` : '')
</script>
