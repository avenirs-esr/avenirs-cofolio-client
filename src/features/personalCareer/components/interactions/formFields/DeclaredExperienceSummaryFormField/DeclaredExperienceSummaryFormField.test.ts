...
import { DeclaredExperienceSummaryTextareaStub } from '@/features/personalCareer/components/interactions/inputs/DeclaredExperienceSummaryTextarea/DeclaredExperienceSummaryTextarea.stub'
import {
...
import { DECLARED_EXPERIENCE_SUMMARY_MAX_LENGTH } from '@/features/personalCareer/config'
import { BddTest } from '@avenirs-esr/avenirs-dsav/test-utils'
import { mount, type VueWrapper } from '@vue/test-utils'
import { createFormFieldTestWrapper } from 'tests/utils'
import { beforeEach, expect, vi } from 'vitest'

const TestWrapper = createFormFieldTestWrapper<DeclaredExperienceFormData, 'summary'>({
  ...
  BddTest().when('the component is mounted', () => {
    ...
    BddTest().and('the user enters a summary exceeding max length', () => {
      BddTest().then('it should keep the user value in the field state', async () => {
        const textarea = wrapper.findComponent({ name: 'DeclaredExperienceSummaryTextarea' })
        const textInput = textarea.find('textarea')
        const longSummary = 'a'.repeat(DECLARED_EXPERIENCE_SUMMARY_MAX_LENGTH + 10)
