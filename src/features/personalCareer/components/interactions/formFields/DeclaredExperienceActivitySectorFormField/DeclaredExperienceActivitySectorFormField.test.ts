...
import { DeclaredExperienceActivitySectorInputStub } from '@/features/personalCareer/components/interactions/inputs/DeclaredExperienceActivitySectorInput/DeclaredExperienceActivitySectorInput.stub'
import {
...
import { DECLARED_EXPERIENCE_ACTIVITY_SECTOR_MAX_LENGTH } from '@/features/personalCareer/config'
import { BddTest } from '@avenirs-esr/avenirs-dsav/test-utils'
import { mount, type VueWrapper } from '@vue/test-utils'
import { createFormFieldTestWrapper } from 'tests/utils'
import { beforeEach, expect, vi } from 'vitest'

const TestWrapper = createFormFieldTestWrapper<DeclaredExperienceFormData, 'activitySector'>({
  ...
  BddTest().when('the component is mounted', () => {
    ...
    BddTest().and('the user enters an activity sector exceeding max length', () => {
      BddTest().then('it should keep the user value in the field state', async () => {
        ...
        await vi.waitFor(() => {
          const updated = wrapper.findComponent({ name: 'DeclaredExperienceActivitySectorInput' })
          expect(updated.props('modelValue')).toBe(longSector)
        })
      })
    })
