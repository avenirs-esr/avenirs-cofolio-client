...
BddTest().given('a declared experience source of information input component', () => {
  let wrapper: VueWrapper<InstanceType<typeof DeclaredExperienceSourceOfInformationInput>>
  ...
  BddTest().when('the component is mounted', () => {
    beforeEach(() => {
      vi.clearAllMocks()
      wrapper = mount(DeclaredExperienceSourceOfInformationInput, {
        props: {
          modelValue: ''
        },
        global: { stubs }
    ...
    BddTest().then('it should have maxlength prop set to config value', () => {
      const input = wrapper.findComponent(InputStub)
      expect(input.props('maxlength')).toBe(DECLARED_EXPERIENCE_SOURCE_OF_INFORMATION_MAX_LENGTH)
    })
  ...
  BddTest().when('the user types in the input', () => {
    beforeEach(async () => {
      ...
      const input = wrapper.findComponent(InputStub)
      await input.vm.$emit('update:modelValue', 'LinkedIn')
      await wrapper.vm.$nextTick()
    })

    BddTest().then('it should update the model value', () => {
      const input = wrapper.findComponent(InputStub)
      expect(input.props('modelValue')).toBe('LinkedIn')
    })
  })
