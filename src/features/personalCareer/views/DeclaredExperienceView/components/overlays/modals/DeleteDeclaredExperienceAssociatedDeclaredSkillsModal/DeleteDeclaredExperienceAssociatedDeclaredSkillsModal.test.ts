...
} from '@/features/personalCareer/views/DeclaredExperienceView/components/overlays/modals/DeleteDeclaredExperienceAssociatedDeclaredSkillsModal/DeleteDeclaredExperienceAssociatedDeclaredSkillsModal.vue'
...
BddTest().given('a delete declared experience associated declared skills modal', () => {
  let wrapper: VueWrapper<InstanceType<typeof DeleteDeclaredExperienceAssociatedDeclaredSkillsModal>>
  ...
  BddTest().when('the modal is shown', () => {
    ...
    BddTest().then('it should map associations to id and title', () => {
      const expectedElements = associations.map(({ associationId, declaredSkill }) => ({
        id: associationId,
        title: declaredSkill.title
      }))
      expect(wrapper.findComponent(CompactCardSelectorStub).props('elements')).toEqual(
        expect.arrayContaining(expectedElements.map(el => expect.objectContaining(el)))
      )
      expect(wrapper.findComponent(DeleteAssociationsModalStub).props('associations')).toEqual(
        expect.arrayContaining(expectedElements.map(el => expect.objectContaining(el)))
      )
    })
    ...
    BddTest().and('deleting associated declared skills fails', () => {
      beforeEach(() => {
