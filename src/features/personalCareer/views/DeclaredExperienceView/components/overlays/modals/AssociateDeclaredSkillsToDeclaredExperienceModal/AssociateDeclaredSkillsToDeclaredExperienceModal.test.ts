...
import { searchDeclaredSkillsForAssociationErrorHandler } from '@/__mocks__/msw/handlers/student/skills.handlers'
...
import AssociateDeclaredSkillsToDeclaredExperienceModal, {
  type AssociateDeclaredSkillsToDeclaredExperienceModalProps
} from '@/features/personalCareer/views/DeclaredExperienceView/components/overlays/modals/AssociateDeclaredSkillsToDeclaredExperienceModal/AssociateDeclaredSkillsToDeclaredExperienceModal.vue'
...
BddTest().given('an associate declared skills to declared experience modal', () => {
  let wrapper: VueWrapper<InstanceType<typeof AssociateDeclaredSkillsToDeclaredExperienceModal>>

  const stubs = {
    AssociateDeclaredSkillsModal: AssociateDeclaredSkillsModalStub
  }

  const props: AssociateDeclaredSkillsToDeclaredExperienceModalProps = {
    show: true,
  ...
  BddTest().when('the modal is rendered', () => {
    ...
    beforeEach(async () => {
      wrapper = mountComponent(AssociateDeclaredSkillsToDeclaredExperienceModal, {
        props,
        global: { stubs }
      })
      await flushPromises()
      modal = wrapper.findComponent(AssociateDeclaredSkillsModalStub)
    })
    ...
    BddTest().when('loading skills fails', () => {
      beforeEach(async () => {
        server.use(searchDeclaredSkillsForAssociationErrorHandler)
      ...
      BddTest().then('it should pass empty skills to the inner modal', () => {
        const modal = wrapper.findComponent(AssociateDeclaredSkillsModalStub)
        expect(modal.props('skills')).toEqual([])
      })
    })

    BddTest().when('associating skills fails', () => {
      beforeEach(async () => {
        server.use(associateDeclaredExperienceWithDeclaredSkillsErrorHandler)
