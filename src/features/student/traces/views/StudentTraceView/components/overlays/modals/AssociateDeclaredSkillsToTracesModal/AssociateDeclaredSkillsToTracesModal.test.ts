...
import {
  associateTraceWithDeclaredSkillsErrorHandler,
  searchSkillsForAssociationErrorHandler
...
import AssociateDeclaredSkillsToTracesModal, {
  type AssociateDeclaredSkillsToTracesModalProps
...
BddTest().given('an associate declared skills to traces modal', () => {
  let wrapper: VueWrapper<InstanceType<typeof AssociateDeclaredSkillsToTracesModal>>

  const stubs = {
    AssociateDeclaredSkillsModal: AssociateDeclaredSkillsModalStub
  }

  const props: AssociateDeclaredSkillsToTracesModalProps = {
    show: true,
    traceId: 'trace-1'
  }
  ...
  BddTest().when('the modal is rendered', () => {
    ...
    beforeEach(async () => {
      wrapper = mountComponent(AssociateDeclaredSkillsToTracesModal, {
        props,
        global: { stubs }
      })
      await flushPromises()
