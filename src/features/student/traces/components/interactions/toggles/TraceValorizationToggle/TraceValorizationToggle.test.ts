...
BddTest().given('a trace valorization toggle component', () => {
  let wrapper: VueWrapper<InstanceType<typeof TraceValorizationToggle>>
  ...
  beforeEach(() => {
    ...
    wrapper = mount(TraceValorizationToggle, {
      props: {
        id: 'trace-valorization',
        name: 'trace-valorization',
        modelValue: false
      },
      global: {
        stubs
  ...
  BddTest().when('the component is mounted', () => {
    ...
    BddTest().then('it should have the default description from i18n', () => {
      const toggle = wrapper.findComponent({ name: 'Toggle' })
      expect(toggle.props('description')).toBe('Je valorise cette trace dans mon kit')
    })
    ...
    BddTest().then('it should pass through name prop', () => {
      const toggle = wrapper.findComponent({ name: 'Toggle' })
      expect(toggle.props('name')).toBe('trace-valorization')
    })
