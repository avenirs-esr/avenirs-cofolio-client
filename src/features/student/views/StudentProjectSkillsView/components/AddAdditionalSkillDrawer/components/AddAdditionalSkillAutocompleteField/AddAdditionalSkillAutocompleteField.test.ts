  ...
  AvAutocomplete: {
    ...
    template: `
      <div class="av-autocomplete-stub">
        ...
        <slot name="empty" />
        <slot name="item" :option="{ id: '1', title: 'Test Skill', pathSegments: ['Path'], type: '${AdditionalSkillType.ROME4}' }" :isSelected="false" :toggle="() => {}" />
      </div>
    `
  },
...
BddTest().given('an autocomplete skill field component', () => {
  ...
  BddTest().when('rendering the skill type badge', () => {
    BddTest().then('it should render AdditionalSkillTypeBadge with correct props', () => {
      const badge = wrapper.findComponent({ name: 'AdditionalSkillTypeBadge' })
      expect(badge.exists()).toBe(true)
