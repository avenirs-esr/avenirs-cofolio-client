...
const {
  ...
  show,
  onClose
} = defineProps<{ trace: TraceViewDTO, show: boolean, onClose: () => void }>()

const { t } = useI18n()

const currentStep = ref(0)
const steps = computed(() => [
  t('student.views.studentToolsTracesView.updateTraceModal.steps.terms.title'),
  t('student.views.studentToolsTracesView.updateTraceModal.steps.update.title')
])
...
const confirmLabel = computed(() => currentStep.value === 0
  ? t('student.views.studentToolsTracesView.updateTraceModal.buttons.validate')
  : t('student.views.studentToolsTracesView.updateTraceModal.buttons.save'))
...
<template>
  <AvModal
    :opened="show"
    :close-button-label="t('student.views.studentToolsTracesView.updateTraceModal.buttons.close').toUpperCase()"
  ...
  >
    <template #header>
      <div class="header">
        <h5 class="n5">
          {{ t('student.views.studentToolsTracesView.updateTraceModal.title') }} <span class="s1-regular">{{ trace.title }}</span>
        </h5>
      </div>
    </template>
