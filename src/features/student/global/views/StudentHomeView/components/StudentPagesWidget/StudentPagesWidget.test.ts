...
vi.mock('@/features/student/global/queries/use-student-pages.query/use-student-pages.query', () => ({
  useStudentPagesSummaryQuery: vi.fn()
}))

const mockedUseStudentPagesQuery = vi.mocked(useStudentPagesSummaryQuery)

function mockUseStudentPagesQuery (payload: PageOverviewDTO[]) {
  ...
  mockedUseStudentPagesQuery.mockReturnValue(queryMockedData)
}
...
BddTest().given('a student pages widget', async () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks()
    mockUseStudentPagesQuery(pages)
  ...
  testUseBaseApiExceptionToast<PageOverviewDTO[]>({
    mockedUseQuery: mockedUseStudentPagesQuery,
    payload: [],
