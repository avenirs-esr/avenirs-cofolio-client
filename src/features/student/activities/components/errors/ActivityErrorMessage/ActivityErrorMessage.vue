...
const { t } = useI18n()
const { isMatchingErrorCode, isNotFound, getErrorMessage } = useApiErrors(error)

const title = computed(() => {
  if (isMatchingErrorCode(ErrorCodes.ACTIVITY_NOT_FOUND) || isNotFound.value) {
    return t('student.activities.errors.ActivityErrorMessage.title')
  }
...
const description = computed(() => {
  if (isMatchingErrorCode(ErrorCodes.ACTIVITY_NOT_FOUND)) {
    return t('student.activities.errors.ActivityErrorMessage.description')
  }
