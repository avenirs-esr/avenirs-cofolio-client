...
import ActivitiesSideNavigation from '@/features/student/activities/views/ActivitiesCatalogView/components/ActivitiesSideNavigation/ActivitiesSideNavigation.vue'
...
BddTest().given('an ActivitiesSideNavigation', () => {
  ...
  BddTest().when('the component is mounted with widget false', () => {
    ...
    BddTest().and('an invalid selected item format is emitted', () => {
      ...
      BddTest().then('it should not navigate', () => {
        expect(navigateToStudentActivitiesCatalogMock).toHaveBeenCalledTimes(0)
        expect(navigateToStudentWidgetActivityCatalogMock).not.toHaveBeenCalled()
      })
    })
  })
