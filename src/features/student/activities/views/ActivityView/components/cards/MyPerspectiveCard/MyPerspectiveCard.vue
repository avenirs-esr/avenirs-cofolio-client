...
function updateActivityReflection (reflection: string) {
  mutateUpdateReflection({ activityId, data: { reflection } }, {
    onError: (error) => {
      addErrorMessage({
        title: t('student.activities.views.ActivityView.MyPerspectiveCard.save.error'),
    ...
    onSuccess: async () => {
      ...
      addSuccessMessage(t('student.activities.views.ActivityView.MyPerspectiveCard.save.success'))
      readonly.value = true
    }
  })
}
...
function updateActivityReflectionOnAutoSave (reflection: string) {
  mutateUpdateReflectionOnAutoSave({ activityId, data: { reflection } }, {
    onError: (error) => {
      addErrorMessage({
        title: t('student.activities.views.ActivityView.MyPerspectiveCard.autoSave.error'),
        description: getErrorMessage(error)
      })
    },
    onSuccess: async () => {
      await withTaskLoading(() => invalidateGetActivityPresentation(queryClient, EActivityStatus.PUBLISHED, activityId))
      addSuccessMessage(t('student.activities.views.ActivityView.MyPerspectiveCard.autoSave.success'))
    }
  })
}
    ...
    <template #title>
      <div class="av-row av-flex-fill av-align-center av-justify-between av-gap-sm">
        <AvIconText
          ...
          :text="t('student.activities.views.ActivityView.MyPerspectiveCard.perspective')"
