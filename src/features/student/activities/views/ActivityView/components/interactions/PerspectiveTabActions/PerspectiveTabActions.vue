...
import FinishDeclaredActivity
  from '@/features/student/activities/views/ActivityView/components/FinishDeclaredActivity/FinishDeclaredActivity.vue'
import RequestFeedback
  from '@/features/student/activities/views/ActivityView/components/interactions/RequestFeedback/RequestFeedback.vue'
...
const actionsHintInfo = computed(() => {
  ...
  if (declaredActivityDetails.finishedAt) {
    return {
      text: t('student.activities.views.ActivityView.FinishDeclaredActivity.finishedAt', {
  ...
  if (lastFeedback.value?.status === EFeedbackStatus.NEW && isDeclaredActivitySubmitted.value) {
    return {
      text: t('student.activities.views.ActivityView.PerspectiveTabActions.updatableFeedback', {
  ...
  if (isDeclaredActivitySubmitted.value) {
    return {
      text: t('student.activities.views.ActivityView.PerspectiveTabActions.feedbackPending'),
  ...
  if (remainingFeedbacks.value === 0) {
    return {
      text: t('student.activities.views.ActivityView.PerspectiveTabActions.maximumFeedbackReached'),
...
function finishDeclaredActivity () {
  mutateFinish({ declaredActivityId: declaredActivityDetails.id }, {
    onError: (error: BaseApiException) => {
      addErrorMessage({
        title: t('student.activities.views.ActivityView.FinishDeclaredActivityConfirmModal.error'),
    ...
    onSuccess: async () => {
      ...
      addSuccessMessage({
        timeout: 2000,
        description: t('student.activities.views.ActivityView.FinishDeclaredActivityConfirmModal.success'),
      })
    },
  })
}
...
function requestFeedback () {
  mutateAskForFeedback({ declaredActivityId: declaredActivityDetails.id }, {
    onError: (error: BaseApiException) => {
      addErrorMessage({
        title: t('student.activities.views.ActivityView.PerspectiveTabActions.error'),
        description: getErrorMessage(error),
      })
    },
    onSuccess: async () => {
      ...
      addSuccessMessage({
        timeout: 2000,
        description: t('student.activities.views.ActivityView.PerspectiveTabActions.success'),
      })
    },
  })
}
