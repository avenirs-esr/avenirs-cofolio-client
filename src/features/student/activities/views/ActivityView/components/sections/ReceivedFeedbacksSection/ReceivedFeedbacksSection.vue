...
import FeedbackCard from '@/features/student/activities/views/ActivityView/components/cards/FeedbackCard/FeedbackCard.vue'
...
const title = computed(() => {
  ...
  return max === -1
    ? t('student.activities.views.ActivityView.ReceivedFeedbacksSection.titleUnlimited', { count })
    : t('student.activities.views.ActivityView.ReceivedFeedbacksSection.titleLimited', { count, max })
})
</script>
  ...
  >
    ...
    >
      <span class="b2-light av-text-text2">
        {{ t('student.activities.views.ActivityView.ReceivedFeedbacksSection.empty') }}
      </span>
    </AvCard>
  </div>
</template>
