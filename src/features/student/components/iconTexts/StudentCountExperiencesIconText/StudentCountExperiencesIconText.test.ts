...
vi.mock('vue-i18n', () => ({
  useI18n: () => ({
    t: (key: string, values: Record<string, any>) => {
      if (key === 'student.iconTexts.studentCountExperiencesIconText.text') {
        return `${values.count} expériences`
      }
      return key
    }
  })
...
describe('studentCountExperiencesIconText.vue', () => {
  ...
  it('should render with default gap if not provided', () => {
  ...
  it('should apply the correct text and icon colors', () => {
    const wrapper = createWrapper({ countExperiences: 4 })
    const iconText = wrapper.findComponent({ name: 'AvIconText' })
