  ...
  BddTest().when('mounted in delete variant with several types', () => {
    ...
    BddTest().then('it should render one menu item per given type, in the given order', () => {
      ...
      expect(items.map((item: { name: string }) => item.name)).toEqual([
        ASSOCIATION_DROPDOWN_ITEM_NAMES[EAssociationContextType.DECLARED_ACTIVITY],
        ASSOCIATION_DROPDOWN_ITEM_NAMES[EAssociationContextType.DECLARED_SKILL],
        ASSOCIATION_DROPDOWN_ITEM_NAMES[EAssociationContextType.TRACE],
        ASSOCIATION_DROPDOWN_ITEM_NAMES[EAssociationContextType.DECLARED_EXPERIENCE],
      ])
    })
    ...
    BddTest().and('an item is clicked', () => {
      beforeEach(async () => {
        await wrapper.find(`[data-name="${ASSOCIATION_DROPDOWN_ITEM_NAMES[EAssociationContextType.DECLARED_SKILL]}"]`).trigger('click')
      })

      BddTest().then('it should emit select with the matching association type', () => {
    ...
    BddTest().and('an unknown item is selected', () => {
      beforeEach(async () => {
        await wrapper.findComponent(AvDropdownStub).vm.$emit('itemSelected', 'unknown')
      })

      BddTest().then('it should not emit select', () => {
