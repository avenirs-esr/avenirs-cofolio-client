...
import AssociationCard, { type AssociationCardProps } from '@/features/student/associations/components/cards/AssociationCard/AssociationCard.vue'
...
BddTest().given('an association card', () => {
  ...
  BddTest().when('the component is mounted with all props and all slots', () => {
    ...
    BddTest().then('it should link to the given route', () => {
  ...
  BddTest().when('the component is mounted disabled', () => {
    const props: AssociationCardProps = {
      title: 'Skill',
      icon: ICONS.SKILLS,
      color: 'var(--color)',
    ...
    BddTest().then('it should not link to the given route', () => {
      expect(wrapper.findComponent(RouterLinkStub).exists()).toBe(false)
      expect(wrapper.find('[data-testid="association-card"]').element.tagName).toBe('DIV')
    })

    BddTest().then('it should render the card as disabled', () => {
      expect(wrapper.find('[data-testid="association-card"]').classes()).toContain('association-card--disabled')
    })

    BddTest().then('it should not highlight the card border on hover', () => {
      expect(wrapper.findComponent(FloatingIconCardStub).props('borderColorOnHover')).toBe('transparent')
    })
  })
})
