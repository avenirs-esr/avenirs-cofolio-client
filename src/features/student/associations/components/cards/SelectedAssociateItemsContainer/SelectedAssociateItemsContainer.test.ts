  ...
  BddTest().when('the component is mounted with a scoped item slot', () => {
    beforeEach(() => {
      wrapper = mountComponent(SelectedAssociateItemsContainerComponent, {
        ...
        global: { stubs },
        slots: {
          item: '<template #item="{ item }"><span data-testid="item-slot-title">{{ item.title }}</span></template>'
        }
      })
    })

    BddTest().then('it should render the slot of every item with the item', () => {
      const titles = wrapper.findAll('[data-testid="item-slot-title"]')
      expect(titles.map(title => title.text())).toEqual(['Item 1', 'Item 2'])
    })
  })
