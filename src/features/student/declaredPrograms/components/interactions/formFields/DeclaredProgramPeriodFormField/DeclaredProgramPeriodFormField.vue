...
<template>
  <div class="declared-program-period-form-field">
    <div class="b2-light av-py-xs">
      {{ t('student.declaredPrograms.interactions.formFields.DeclaredProgramPeriodFormField.label') }}
    </div>
      ...
      <div class="av-row-md">
        <FormField name="startDate">
          <template #default="{ field }">
            <AvInput
              ...
              :label="t('student.declaredPrograms.interactions.formFields.DeclaredProgramPeriodFormField.startDate')"
              :label-visible="false"
              :placeholder="t('student.declaredPrograms.interactions.formFields.DeclaredProgramPeriodFormField.startDatePlaceholder')"
        ...
        <FormField name="endDate">
          <template #default="{ field }">
            <AvInput
              :model-value="field.state.value"
              :label="t('student.declaredPrograms.interactions.formFields.DeclaredProgramPeriodFormField.endDate')"
              :placeholder="t('student.declaredPrograms.interactions.formFields.DeclaredProgramPeriodFormField.endDatePlaceholder')"
              :error-message="field.state.meta.errors?.join(', ')"
              :prefix-icon="MDI_ICONS.CALENDAR_MONTH_OUTLINE"
              type="month"
              :disabled="isOngoing"
