...
import type { UsePaginatedStaffFeedbacksParams, UsePaginatedStaffFeedbacksResult } from '@/features/feedbacks/composables/use-paginated-staff-feedbacks/use-paginated-staff-feedbacks'
...
import { FeedbackStatusBadgeStub } from '@/features/feedbacks/components/badges/FeedbackStatusBadge/FeedbackStatusBadge.stub'
import { FeedbackCardStub } from '@/features/feedbacks/views/FeedbacksView/components/cards/FeedbackCard/FeedbackCard.stub'
...
vi.mock('@/features/feedbacks/composables/use-paginated-staff-feedbacks/use-paginated-staff-feedbacks', () => ({
  usePaginatedStaffFeedbacks: vi.fn(),
}))

const { usePaginatedStaffFeedbacks: mockedUsePaginatedStaffFeedbacks } = await import('@/features/feedbacks/composables/use-paginated-staff-feedbacks/use-paginated-staff-feedbacks')
...
BddTest().given('a FeedbacksTable component', () => {
  ...
  BddTest().when('withActivity is false', () => {
    beforeEach(async () => {
      await mountDefault({ props: { withActivity: false } })
    })

    BddTest().then('it should not render the activity column', () => {
      const columns = wrapper.findComponent(AvTableStub).props('columns') as AvTableColumn<FeedbackStaffListItemDTO & { access?: string }>[]
      expect(columns.map(c => c.key)).not.toContain('activity')
    })
  })
