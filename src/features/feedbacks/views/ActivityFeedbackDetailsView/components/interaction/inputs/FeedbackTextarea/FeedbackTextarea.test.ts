...
import { FEEDBACK_MAX_LENGTH } from '@/features/feedbacks/config'
...
BddTest().given('a feedback textarea component', () => {
  let wrapper: VueWrapper<InstanceType<typeof FeedbackTextarea>>
  ...
  BddTest().when('the component is mounted with model value', () => {
    beforeEach(() => {
      vi.clearAllMocks()
      wrapper = mount(FeedbackTextarea, {
        props: {
          modelValue: '',
        },
        global: { stubs }
    ...
    BddTest().then('it should have maxlength prop set to config value', () => {
      const input = wrapper.findComponent(InputStub)
      expect(input.props('maxlength')).toBe(FEEDBACK_MAX_LENGTH)
    })
    ...
    BddTest().then('it should display zero character count', () => {
      expect(wrapper.text()).toContain(`0 / ${FEEDBACK_MAX_LENGTH}`)
    })
  })
  ...
  BddTest().when('the textarea has long text content', () => {
    ...
    BddTest().then('it should display correct character count', () => {
      expect(wrapper.text()).toContain(`50 / ${FEEDBACK_MAX_LENGTH}`)
    })
  })
