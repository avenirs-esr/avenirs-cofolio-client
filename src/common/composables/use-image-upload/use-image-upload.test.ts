...
BddTest().given('an useImageUpload composable', () => {
  ...
  BddTest().when('initialized', () => {
    BddTest().then('all refs should be undefined or null', () => {
    ...
    BddTest().then('all methods should be available', () => {
      expect(typeof imageUpload.update).toBe('function')
      expect(typeof imageUpload.clear).toBe('function')
    })
  })

  BddTest().when('update is called with an oversized file', () => {
    beforeEach(() => {
      const fileList = {
        0: largeFile,
        length: 1,
    ...
    BddTest().then('it should set an error and not set previewUrl or name', () => {
  ...
  BddTest().when('update is called with a valid file', () => {
    let fileUrl: string
    ...
    BddTest().then('it should set name, valid, and previewUrl, and clear error', () => {
      expect(imageUpload.error.value).toBeUndefined()
  ...
  BddTest().when('update is called with an empty FileList', () => {
    beforeEach(() => {
      const emptyFileList = {
        0: undefined,
        length: 0,
    ...
    BddTest().then('it should not set any error, name, valid or previewUrl', () => {
      expect(imageUpload.error.value).toBeUndefined()
      expect(imageUpload.valid.value).toBeUndefined()
      expect(imageUpload.name.value).toBeUndefined()
      expect(imageUpload.previewUrl.value).toBeNull()
  ...
  BddTest().when('clear is called', () => {
    ...
    BddTest().then('all refs should be reset and URL revoked', () => {
      expect(imageUpload.name.value).toBeUndefined()
      expect(imageUpload.valid.value).toBeUndefined()
      expect(imageUpload.error.value).toBeUndefined()
      expect(imageUpload.previewUrl.value).toBeNull()
