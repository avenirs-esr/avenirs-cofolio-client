...
import { ActivityCard } from '@e2e/framework/student/activities/activitiesPages/componentObjects/ActivityCard'
...
export class AllActivitiesTabs extends BaseObject {
  ...
  async verifyHeaderDescription () {
    const expectedText = t('student.activities.views.ActivitiesView.AllActivitiesHeaderCard.description')
    await expect(this.getHeaderDescription()).toBeVisible()
    await expect(this.getHeaderDescription()).toHaveText(expectedText)
  }

  async verifySeeAllButton () {
    const expectedText = t('student.activities.views.ActivitiesView.AllActivitiesHeaderCard.seeAll')
    await expect(this.getHeaderSeeAllButton()).toBeVisible()
