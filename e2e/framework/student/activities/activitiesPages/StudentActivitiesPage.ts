...
import type { StudentActivitiesCatalogPage } from '@e2e/framework/student/activities/activitiesCatalogPage/StudentActivitiesCatalogPage'
import type { Page } from '@playwright/test'
import { BasePage } from '@e2e/framework/shared/base/BasePage'
import { STUDENT_ROUTES } from '@e2e/framework/shared/constants/routes'
import { ActivityLibraryTab } from '@e2e/framework/student/activities/activitiesPages/componentObjects/ActivityLibraryTab'
import { AllActivitiesTabs } from '@e2e/framework/student/activities/activitiesPages/componentObjects/AllActivitiesTabs'
import { Fixture, Given, Then, When } from 'playwright-bdd/decorators'

export
@Fixture<typeof test>('studentActivitiesPage')
class StudentActivitiesPage extends BasePage {
  ...
  async restoreActivitySubscription (catalogPage: StudentActivitiesCatalogPage) {
  ...
  async clickFirstLibraryActivityCard () {
    ...
    await this.page.waitForURL(new RegExp(STUDENT_ROUTES.ACTIVITY.replace(':id', '.+')))
  }
  ...
  async clickInProgressActivityWithoutReceivedFeedbacks () {
    ...
    await this.getActivityLibraryTab().clickCardByActivityId(
      StudentActivitiesPage.ACTIVITY_ID_WITHOUT_RECEIVED_FEEDBACKS
    )
  ...
  async clickInProgressActivityWithAssociatedTraces () {
    await this.getActivityLibraryTab().clickCardByActivityId(
      StudentActivitiesPage.ACTIVITY_ID_WITH_ASSOCIATED_TRACES
    )
