...
import { ActivityDetailsObject } from '@e2e/framework/student/activities/activityPage/componentObjects/ActivityDetailsObject'
import { MyPerspectiveSectionObject } from '@e2e/framework/student/activities/activityPage/componentObjects/MyPerspectiveSectionObject'
import { expect, type Page } from '@playwright/test'
import { Fixture, Then, When } from 'playwright-bdd/decorators'

@Fixture<typeof test>('studentActivityPage')
export class StudentActivityPage extends BasePage {
  constructor (public page: Page) {
    super(page)
  }

  getActivityDetails () {
    return new ActivityDetailsObject(this.page.getByTestId('activity-details'))
  }
  ...
  getTitle () {
    return this.page.getByTestId('activity-view-title')
  }

  getStatus () {
    return this.page.getByTestId('activity-view-status-badge')
  }

  getDropdown () {
    return this.page.getByTestId('activity-view-detailed-dropdown')
  }
  ...
  @Then('the activity details is loaded')
  async verifyActivityDetailsVisible () {
    await this.getActivityDetails().verifyVisible()
  }
  ...
  async verifyStartDateVisible () {
    await this.getActivityDetails().verifyPeriodInputVisible()
  }

  @Then('the activity details end date is visible')
  ...
  async verifyActivityTitleVisible () {
    await this.getActivityDetails().verifyTitleVisible()
  }

  @Then('the activity details description is visible')
  async verifyActivityDescriptionVisible () {
    await this.getActivityDetails().verifyDescriptionVisible()
  }
  ...
  async verifyRecommendedCompletionContextsListVisible () {
    await this.getActivityDetails().verifyRecommendedCompletionContextsListVisible()
  }
