...
import { ActivitiesPreviousNextNavigation } from '@e2e/framework/student/activities/activitiesCatalogPage/componentObjects/ActivitiesPreviousNextNavigation'
import { ActivitiesSelectNavigation } from '@e2e/framework/student/activities/activitiesCatalogPage/componentObjects/ActivitiesSelectNavigation'
import { ActivitiesSideNavigation } from '@e2e/framework/student/activities/activitiesCatalogPage/componentObjects/ActivitiesSideNavigation'
import { ActivityPreview } from '@e2e/framework/student/activities/activitiesCatalogPage/componentObjects/ActivityPreview'
import { expect, type Page } from '@playwright/test'
import { Fixture, Given, Then, When } from 'playwright-bdd/decorators'

export
@Fixture<typeof test>('studentActivitiesCatalogPage')
class StudentActivitiesCatalogPage {
  ...
  private buildCatalogUrl (thematic: string, id: string) {
    return STUDENT_ROUTES.ACTIVITIES_CATALOG
  ...
  async unsubscribeFromDefaultActivity () {
    const url = this.buildCatalogUrl(
      StudentActivitiesCatalogPage.DEFAULT_THEMATIC,
      StudentActivitiesCatalogPage.DEFAULT_ACTIVITY_ID,
    )
    await this.page.goto(url)
  ...
  async verifyRedirectedToActivityDetailedPage () {
    ...
    const expectedUrl = STUDENT_ROUTES.ACTIVITY.replace(':id', this.subscribedDeclaredActivity!)
